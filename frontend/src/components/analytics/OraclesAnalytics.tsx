        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl px-4 py-3">
                    <p className="text-xs uppercase tracking-wider text-[var(--color-text-muted)] mb-1">Total Oracles</p>
                    <p className="text-xl font-bold text-[var(--color-btc-orange)]">{totalOracles}</p>
                </div>
                <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl px-4 py-3">
                    <p className="text-xs uppercase tracking-wider text-[var(--color-text-muted)] mb-1">Markets Resolved</p>
                    <p className="text-xl font-bold text-green-400">{totalResolved}</p>
                </div>
                <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl px-4 py-3">
                    <p className="text-xs uppercase tracking-wider text-[var(--color-text-muted)] mb-1">Pending Resolution</p>
                    <p className="text-xl font-bold text-[var(--color-btc-orange)]">{totalPending}</p>
                </div>
                <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl px-4 py-3">
                    <p className="text-xs uppercase tracking-wider text-[var(--color-text-muted)] mb-1">Avg Markets/Oracle</p>
            ...
            <div className="grid md:grid-cols-2 gap-6">
                <Card>
                    <h3 className="text-sm font-semibold text-[var(--color-text-primary)] mb-4">Top Oracles</h3>
                    {topOracles.length > 0 ? (
                ...
                <Card>
                    <h3 className="text-sm font-semibold text-[var(--color-text-primary)] mb-4">Resolution Outcomes</h3>
                ...
                <Card>
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-sm font-semibold text-[var(--color-text-primary)]">
                            Oracle Profile: <span className="text-[var(--color-btc-orange)] font-mono">{truncateAddr(expandedOracleData.address)}</span>
                        </h3>
                        <button
                            onClick={() => setExpandedOracle(null)}
                            className="text-xs text-[var(--color-text-secondary)] hover:text-[var(--color-btc-orange)] transition-colors cursor-pointer"
                        >
                            Close
                        </button>
                    </div>
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between text-sm">
                                <span className="text-[var(--color-text-secondary)]">Full Address</span>
                                <span className="text-[var(--color-text-primary)] font-mono text-xs break-all">{expandedOracleData.address}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-[var(--color-text-secondary)]">Markets Served</span>
                                <span className="text-[var(--color-text-primary)] font-bold">{expandedOracleData.marketsServed}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                            ...
                            <div className="flex justify-between text-sm">
                                ...
                                <span className="text-[var(--color-btc-orange)] font-bold">{expandedOracleData.marketsPending}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-[var(--color-text-secondary)]">Total Volume</span>
                                <span className="text-[var(--color-btc-orange)] font-bold">{formatSats(expandedOracleData.totalVolume)}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-[var(--color-text-secondary)]">Avg Pool Size</span>
                                <span className="text-[var(--color-text-primary)]">{formatSats(expandedOracleData.avgPoolSize)}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-[var(--color-text-secondary)]">YES / NO Ratio</span>
                    ...
                    <div className="mt-4 border-t border-[var(--color-border)] pt-4">
                        <h4 className="text-xs uppercase tracking-wider text-[var(--color-text-muted)] mb-3">Markets</h4>
                                ...
                                <div key={m.id.toString()} className="flex justify-between text-xs text-[var(--color-text-secondary)]">
                                    ...
                                    <span className="shrink-0 ml-2">
                                        <span className={`font-bold ${
                                            m.status === MarketStatus.RESOLVED ? 'text-[var(--color-text-secondary)]' : 'text-green-400'
                                        }`}>
                            ...
                            {filtered.map((o) => (
                                <tr
                                    ...
                                    className={`border-b border-[var(--color-border)]/50 hover:bg-[var(--color-bg-card-hover)] cursor-pointer transition-colors ${
                                        expandedOracle === o.address ? 'bg-[var(--color-bg-card-hover)]' : ''
                                    }`}
                                >
                                    ...
                                    <td className="py-2.5 pr-3 text-right text-[var(--color-btc-orange)]">{o.marketsServed}</td>
                                    <td className="py-2.5 pr-3 text-right text-green-400">{o.marketsResolved}</td>
                                    <td className="py-2.5 pr-3 text-right text-[var(--color-btc-orange)]">{o.marketsPending}</td>
                                    <td className="py-2.5 pr-3 text-right text-[var(--color-text-primary)]">{formatSats(o.totalVolume)}</td>
